...
- кнопки на дашборді: `Оренда` (модалка керування орендою) і `Пошук трекерів` (on/off);
...
- **Неактивна кнопка "Пошук трекерів" на дашборді**: перевірити, що доступні dashboard endpoints `/dashboard/settings/*`.
